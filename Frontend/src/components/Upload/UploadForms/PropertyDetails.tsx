...
import BenefitListing from "../../BenefitListing";
...
function PropertyDetails({
  ...
  const { setValue, watch, trigger,getValues } = useFormContext<Property>();
  const propertyDetails = watch();
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);
  const choosenBenefits = watch("benefits") || [];

  const handleBenefitClick = (benefit: Benefit) => {
    const exists = choosenBenefits.some((b) => b.name === benefit.name);
  
    const newBenefits = exists
      ? choosenBenefits.filter((b) => b.name !== benefit.name)
      : [...choosenBenefits, { name: benefit.name, icon: benefit.icon }];
  
    setValue("benefits", newBenefits);
    console.log(getValues("benefits"))
  };
  ...
  return (
    <fieldset className="fieldset mx-auto my-2 rounded-box flex justify-center border">
      <div className="flex flex-col w-250">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {conditions.slice(0, 4).map((condition) => (
                      <button
                        type="button"
                        key={condition}
                        className={`btn btn-sm ${propertyDetails.condition === condition ? `btn-success` : "btn-outline"
                          }`}
                        onClick={() => {
                          ...
                          >
                            {conditions.slice(4).map((condition) => (
                              <button
                                type="button"
                                key={condition}
                                className={`block w-full text-left px-4 py-2 btn ${propertyDetails.condition === condition ? `btn-success` : ""
                                  }`}
              ...
              <div className="flex flex-wrap gap-2">
                {propertyTypes.map((propertyType) => (
                  <button
                    type="button"
                    key={propertyType}
                    className={`btn btn-sm ${propertyDetails.property_type === propertyType ? `btn-success` : "btn-outline"
                      }`}
                    onClick={() => {
        ...
        <legend className="fieldset-legend flex justify-center">Benefits:</legend>
        <div className="flex gap-2 justify-center pb-2 flex-wrap">
            <BenefitListing benefits={benefits} choosenBenefits={choosenBenefits} handleBenefitClick={handleBenefitClick} className="flex gap-2 justify-center pb-2 flex-wrap" text={true}></BenefitListing>
        </div>
      </div>
    </fieldset>
  );
